...
pub fn render_with_loading(f: &mut Frame, area: Rect, mem: &MemoryData, data_loaded: bool) {
    ...
    let gauge_area = Rect { y: area.y + 1, height: 1, ..area };
    ...
    if area.height >= 3 && mem.swap_total > 0 {
        let swap_area = Rect { y: area.y + 2, height: 1, ..area };
        ...
        let swap_line = if data_loaded {
            Line::from(vec![
                ...
                Span::styled(
                    format!("  {:.1}%  {} / {}", swap_pct, ByteSize(mem.swap_used), ByteSize(mem.swap_total)),
