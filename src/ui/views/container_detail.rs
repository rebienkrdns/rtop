...
pub fn render(f: &mut Frame, area: Rect, container: &ContainerData, confirm: Option<&ConfirmAction>, state: &AppState) {
    ...
    if state.history_mode {
        let cpu_data: Vec<u64> = state.container_history.iter()
            .skip(state.container_history.len().saturating_sub(state.history_range.samples()))
            .map(|s| s.cpu_pct as u64)
            .collect();
        let cpu_block = Block::default()
            .title(Span::styled(format!(" CPU Historial ({}) · Último: {:.1}% ", state.history_range.label(), cpu_pct), Style::default().fg(theme.muted)))
        ...
        let cpu_spark = Sparkline::default()
            .data(&cpu_data)
            .max(100)
            .style(Style::default().fg(Theme::color_for_pct(cpu_pct)).bg(Color::DarkGray));
    ...
    if state.history_mode {
        let mem_data: Vec<u64> = state.container_history.iter()
            .skip(state.container_history.len().saturating_sub(state.history_range.samples()))
            .map(|s| s.mem_pct as u64)
            .collect();
        let mem_block = Block::default()
            .title(Span::styled(format!(" Memoria Historial ({}) · Último: {:.1}% ", state.history_range.label(), mem_pct), Style::default().fg(theme.muted)))
            .borders(Borders::ALL).border_style(Style::default().fg(theme.muted));
        ...
        let mem_spark = Sparkline::default()
            .data(&mem_data)
            .max(100)
            .style(Style::default().fg(Theme::color_for_pct(mem_pct)).bg(Color::DarkGray));
    ...
    if state.history_mode {
        let net_data: Vec<u64> = state.container_history.iter()
            .skip(state.container_history.len().saturating_sub(state.history_range.samples()))
            .map(|s| s.net_recv_bps.max(s.net_sent_bps) as u64)
            .collect();
        let net_max = state.container_history.iter()
            .skip(state.container_history.len().saturating_sub(state.history_range.samples()))
        ...
        let net_block = Block::default()
            .title(Span::styled(
                format!(" Red Historial ({}) · Último: ↓{}/s · ↑{}/s ", state.history_range.label(), ByteSize(net_recv as u64), ByteSize(net_sent as u64)),
    ...
    if state.history_mode {
        let disk_data: Vec<u64> = state.container_history.iter()
            .skip(state.container_history.len().saturating_sub(state.history_range.samples()))
            .map(|s| s.disk_read_bps.max(s.disk_write_bps) as u64)
            .collect();
        let disk_max = state.container_history.iter()
            .skip(state.container_history.len().saturating_sub(state.history_range.samples()))
        ...
        let disk_block = Block::default()
            .title(Span::styled(
                format!(" Disco Historial ({}) · Último: R:{}/s · W:{}/s ", state.history_range.label(), ByteSize(disk_r as u64), ByteSize(disk_w as u64)),
                Style::default().fg(theme.muted),
            ))
            .borders(Borders::ALL).border_style(Style::default().fg(theme.muted));
